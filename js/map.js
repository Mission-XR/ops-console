/* ============================================================
   map.js — Map Canvas & Agent Rendering
   Handles: 2D map, mission zones, agent markers, tooltips
   WebXR integration point: xr-engine.js will extend this
   ============================================================ */

// ============================================================
//  AGENT RENDERING & TOOLTIP
// ============================================================
...
function renderAgents() {
  ...
  ags.forEach(function(ag){
    ...
    marker.style.left=ag.x+'%'; marker.style.top=ag.y+'%';
...
function showAgentTooltip(ag, e) {
  ...
  tt.id='agent-tooltip';
  var batRow=ag.battery!==null?'<div class="agent-tooltip-row"><span class="agent-tooltip-key">BATTERY</span><span class="agent-tooltip-val">'+ag.battery+'%</span></div>':'';
...
function initCanvas(){
  ...
  function draw(){
    ...
    // Grid
    ctx.strokeStyle='rgba(0,60,90,0.4)'; ctx.lineWidth=0.5;
    ...
    if(m.status==='running'){
      ...
      if(ag){
        var cx=(ag.x/100)*W, cy=(ag.y/100)*H;
    ...
    canvasT++; requestAnimationFrame(draw);
  }
  draw();
}
...
function drawMissionZones(ctx,W,H){
  ...
  if(m.id==='INFRA-INSPECT-01'){
    ...
    var buildings=[{x:.15,y:.2,w:.2,h:.25,label:'BUILDING A'},{x:.55,y:.2,w:.2,h:.25,label:'BUILDING B'},{x:.35,y:.6,w:.22,h:.25,label:'BUILDING C'}];
    buildings.forEach(function(b){
      ...
      ctx.fillStyle='r  gba(0,212,255,0.5)'; ctx.font='9px Share Tech Mono,monospace';
    ...
    ctx.fillStyle='rgba(255,43,94,0.5)';
    ctx.fillText('PROTECTED ZONE',W*.56,H*.57);
    ctx.fillStyle='rgba(0,212,255,0.04)'; ctx.strokeStyle='rgba(0,212,255,0.15)';
    ctx.beginPath(); ctx.rect(W*.1,H*.5,W*.2,H*.35); ctx.fill(); ctx.stroke();
    ctx.fillStyle='rgba(0,212,255,0.5)';
    ctx.fillText('SAFE CORRIDOR',W*.11,H*.52);
  } else if(m.id==='WAREHOUSE-LOG-04'){
    var zones=[{x:.1,y:.15,w:.35,h:.3,label:'ZONE A',col:'green'},{x:.55,y:.15,w:.35,h:.3,label:'ZONE B',col:'yellow'},{x:.1,y:.55,w:.35,h:.3,label:'ZONE C',col:'dim'},{x:.55,y:.55,w:.35,h:.3,label:'ZONE D — FORKLIFT',col:'warn'}];
    ...
    // Sea background
    ctx.fillStyle='rgba(0,20,40,0.4)'; ctx.fillRect(0,0,W,H);
    // Turbines
    ...
    turbines.forEach(function(t,i){
      ...
      [0,120,240].forEach(function(deg){
        ...
        ctx.beginPath(); ctx.moveTo(W*t.x,H*t.y);
    ...
    // --- LÍNEAS ARREGLADAS ---
    ctx.fillStyle='rgba(0,212,255,0.5)';
    ctx.fillText('PIER ACCESS',W*.06,H*.72);
  }
} // <-- ESTA LLAVE CIERRA LA FUNCIÓN drawMissionZones