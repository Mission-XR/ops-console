// ============================================================
//  MAP — 2D Tactical Radar Canvas
//  Reads agent positions from global MISSIONS (updated by WS)
//  Draws zones, paths, agents, tooltips every frame
// ============================================================
var mapCanvas, ctx;
var agentHitboxes = [];
...
function initCanvas() {
    ...
    (function mapLoop() { renderTacticalMap(); requestAnimationFrame(mapLoop); })();
}
...
// ── Main render (runs every frame via requestAnimationFrame) ──
function renderTacticalMap() {
    ...
    m.agents.forEach(function(ag) {
        var px = (ag.posX/100)*cw, py = (ag.posY/100)*ch;
        // Original start position for path drawing
        var sx = (ag.x/100)*cw, sy = (ag.y/100)*ch;

        // Draw path trail for running/planned agents
        if (ag.state === 'running' || ag.state === 'planned') {
            var colorPath = ag.state === 'running' ? 'rgba(0,255,157,0.3)' : 'rgba(255,170,0,0.3)';
            var colorAccent = ag.state === 'running' ? '#00ff9d' : '#ffaa00';
            // Simple line from current position extending in movement direction
            ctx.beginPath(); ctx.moveTo(px, py);
            // Draw a trailing effect
            var trailX = px + (Math.random()-0.5)*2;
            var trailY = py + (Math.random()-0.5)*2;
            ctx.lineTo(trailX, trailY);
            ctx.strokeStyle = colorPath; ctx.lineWidth = 4; ctx.stroke();
        }

        var displayState = (typeof emergencyStopped !== 'undefined' && emergencyStopped) ? 'blocked' : ag.state;
...
function drawAgentIcon(x, y, id, state, iconEmoji, isSelected) {
    var color = state==='running'?'#00ff9d': state==='blocked'?'#ff2a2a': state==='planned'?'#ffaa00':'#00d4ff';
...
function drawAgentTooltip(agent) {
    var cw = mapCanvas.width, ch = mapCanvas.height;
    var px = (agent.posX/100)*cw, py = (agent.posY/100)*ch;
    ...
    ctx.fillStyle = '#00d4ff'; ctx.font = "bold 11px 'Share Tech Mono'"; ctx.textAlign = 'left';
    ...
    ctx.fillText('STATE: ', x+8, y+50); ctx.fillStyle = sc; ctx.fillText(agent.state.toUpperCase(), x+45, y+50);
