...
//  + AudioFX hooks for emergency alarm + red flash
// ============================================================
var WS = {
    ...
    // ── AudioFX helper: keep the alarm/flash in sync with state ──
    _syncEmergencyFX: function(prev){
        if(typeof AudioFX === 'undefined') return;
        if(emergencyStopped && !prev){
            AudioFX.alarm();
            AudioFX.flashEmergency(true);
        } else if(!emergencyStopped && prev){
            AudioFX.stopAlarm();
            AudioFX.flashEmergency(false);
        }
    },
    ...
    handle: function(msg){
        switch(msg.type){
        case 'state':
            var _prevStop = emergencyStopped;
        ...
        case 'emergency':
            var _prevStop2 = emergencyStopped;
            emergencyStopped=msg.stopped; updateEmergencyButton(); renderPanels();
            WS._syncEmergencyFX(_prevStop2);
            break;
